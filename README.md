# Header - README Template

<a class="anchor" id="about_the_project"></a>
<blockquote><h2>About the Project</h2></blockquote>
This is a project on "how to" formatting a README.md file. My preference is using html code

<a class="anchor" id="tools"></a>
><h2>Tools</h2>
These are the tools used in this project:
<ol>
    <li>Jupyter Notebook</li>
    <li>MySQL</li>
    <li>Matplotlib</li>
</ol>

<a class="anchor" id="installation_instructions""></a>
><h2>Installation Instructions</h2>
These are the tools used in this project:
<ol>
    <li>git init</li>
    <li>git add README.md</li>
    <li>git commit -m "first commit"</li>
    <li>git branch -M main</li>
    <li>git remote add origin git@github.com:user/Mini-Lesson-9.4.git</li>
    <li>git push -u origin main</li>
</ol>
<ol>
I did have a lot of authentication issues. So make sure to do the authentication steps correctly, including the following
    <li>eval "$(ssh-agent -s)"</li>
    <li>ssh-add --apple-use-keychain ~/.ssh/your_key</li>
    <li>updating the config file</li>
</ol>

<a class="anchor" id="Contact Information"></a>
><h2>Contact Information</h2>
<p><a href="https://www.linkedin.com/in/carlos-espinoza-859b9838?lipi=urn%3Ali%3Apage%3Ad_flagship3_profile_view_base_contact_details%3BbxHOgdGXQOmO%2F7HgEY7h0w%3D%3D" title="Carlos Espinoza">Carlos Espinoza</a></p>

<a class="anchor" id="Table of Contents"></a>
 ><h2>Table of Contents</h2>
List of the tools used in the project:
<ul>
   <li><a href="#about_the_project">About the Project</a></li>
   <li><a href="#tools">Tools</a></li>
   <li><a href="#installation_instructions">Installation Instructions</a></li>
   <li><a href="#contact">Contact Information</a></li>
</ul>

Github is really awesome!
<img src="https://github.githubassets.com/images/modules/logos_page/Octocat.png" alt="Github Logo - html" title="Github Logo - html" />